...
module Eval = struct
  ...
  let unmarshal_envs : Value.unmarshal_envs ref = ref (Value.empty_env IntMap.empty, IntMap.empty, IntMap.empty, IntMap.empty)
       ...
       let callback rt env conn req body =
              ...
              begin
                ...
                let to_ir_bindings =
                  let to_ir_binding (name, (params, body)) =
                    `Fun (binder_of_name name, ([], List.map binder_of_name params, body), `Unknown) in
                ...
                let response =
                  try
                    Irtojs.generate_program_page ~cgi_env:cgi_args
                                                 ~fix_pickles:false (* What can go wrong? *)
                                                 (Value.get_closures env', Lib.nenv, Lib.typing_env)
                                                 ir_program
                  with
         ...
         | `ExprEval (f, expr_locals) ->
            begin
              ...
              try (
                apply (!render_cont env :: cont) env (f, []);
                assert false
              ) with
              | TopLevel (_, `List [`XML body]) ->
                 Lib.cohttp_server_response [] (Value.string_of_value (`XML body))
